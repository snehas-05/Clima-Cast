...
class SearchService:
    @staticmethod
    async def perform_search(db: Session, query: str, city: Optional[str] = "Ludhiana") -> Dict[str, Any]:
        ...
        weather_data = await WeatherService.get_current_weather(db, target_city)
        forecast_data = await WeatherService.get_forecast(db, target_city)
        ...
        # 3. Intent matching
        if "min" in query_lower and ("temp" in query_lower or "temperature" in query_lower):
            min_temp = current.get("temp_min", "N/A")
            response["answer"] = f"The minimum temperature in {target_city} today is {min_temp}°C."
            ...
            max_temp = current.get("temp_max", "N/A")
            response["answer"] = f"The maximum temperature in {target_city} today is {max_temp}°C."
            response["related_questions"] = [
            ...
            response["answer"] = f"Currently in {target_city}: {current['temp']}°C with {current['condition']}."
