...
const layers = [
  { icon: 'rainy', label: 'Precipitation', active: true },
  { icon: 'air', label: 'Wind Stream' },
  { icon: 'thermostat', label: 'Temperature' },
  { icon: 'satellite_alt', label: 'Satellite View' },
];
    ...
    <div className="relative w-full h-[calc(100vh-80px)] overflow-hidden bg-slate-100">
      ...
      <div className="absolute top-24 right-6 lg:right-10 flex flex-col gap-4 z-20">
        ...
        <div className="glass-panel flex flex-col rounded-2xl overflow-hidden self-end shadow-lg">
          <button className="p-3 hover:bg-white border-b border-white/50 text-primary transition-all">
            <span className="material-symbols-outlined">add</span>
          </button>
          <button className="p-3 hover:bg-white text-primary transition-all">
      ...
      <div className="absolute bottom-28 lg:bottom-10 left-6 lg:left-10 z-20">
        <div className="glass-panel p-6 rounded-[32px] w-80 shadow-2xl">
          ...
          <div className="flex flex-col gap-4">
            {storms.map((s) => (
              <div key={s.name} className={`flex items-center gap-4 p-3 rounded-2xl border border-white/20 bg-white/40 ${s.opacity || ''}`}>
      ...
      <div className="absolute bottom-6 lg:bottom-10 right-6 lg:right-10 left-6 lg:left-[340px] z-20">
        <div className="glass-panel p-4 rounded-[24px] shadow-2xl flex flex-col sm:flex-row items-center gap-6">
          <button className="w-12 h-12 rounded-full bg-primary text-white flex items-center justify-center hover:scale-105 transition-all shrink-0">
            <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>play_arrow</span>
          </button>
          <div className="flex-1 w-full">
            <div className="flex justify-between mb-2">
              <span className="text-xs text-label-caps text-on-surface-variant">Live Radar Playback</span>
            ...
            <div className="h-2 bg-secondary-container/20 rounded-full relative overflow-hidden">
              <div className="absolute top-0 left-0 h-full w-[65%] bg-gradient-to-r from-primary to-[#00BCD4] rounded-full" />
